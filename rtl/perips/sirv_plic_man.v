...
localparam PLIC_PEND_ARRAY = (((PLIC_IRQ_NUM-1)/32) + 1);
 ...
 endgenerate
   ...
   endgenerate
	...
	reg icb_cmd_rd_hsked_r;
	always @ (posedge clk)
	if(!rst_n)
		icb_cmd_rd_hsked_r <= 0;
	else
		icb_cmd_rd_hsked_r <= icb_cmd_rd_hsked;
   ...
   for(i=0; i<PLIC_IRQ_NUM;i=i+1) begin: claim_complete_gen//{
     ...
     always @* begin:claim_complete//{ 
         ...
         icb_claim_irq   [i] = icb_claim_irq[i] | ((icb_rsp_rdata == i) & icb_cmd_sel_clam & icb_cmd_rd_hsked_r);
